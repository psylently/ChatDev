...
@app.route("/")
def index():
    logging.info("should load index.html")
    return send_from_directory("static", "index.html")


@app.route("/chain_visualizer")
def chain_visualizer():
    logging.info("should load /chain_visualizer")
    return send_from_directory("static", "chain_visualizer.html")


@app.route("/replay")
def replay():
    logging.info("should load /replay")
    return send_from_directory("static", "replay.html")


@app.route("/get_messages")
def get_messages():
    logging.info("should load /get_messages")
    return jsonify(messages)


@app.route("/send_message", methods=["POST"])
def send_message():
    logging.info("should load /send_message")
...
if __name__ == "__main__":
    ...
    print(
        f"Please visit http://0.0.0.0:{port[-1]}/ for the front-end display page. \nIn the event of a port"
        " conflict, please modify the port argument (e.g., python3 app.py --port 8012)."
    )
