...
class RolePlaying:
    ...
    def __init__(
        ...
        if with_task_specify:
            ...
            if self.task_type in [TaskType.AI_SOCIETY, TaskType.MISALIGNMENT]:
                task_specify_meta_dict.update(dict(assistant_role=assistant_role_name, user_role=user_role_name))
        ...
        chatdev_prompt_template = (
            "ChatDev is a software company powered by multiple intelligent agents, such as chief executive"
            " officer, chief human resources officer, chief product officer, chief technology officer, etc, with a"
            ' multi-agent organizational structure and the mission of "changing the digital world through'
            ' programming".'
        )
    ...
    def init_chat(self, phase_type: PhaseType = None, placeholders=None, phase_prompt=None):
        ...
        # refactored ChatDev
        content = phase_prompt.format(**({"assistant_role": self.assistant_agent.role_name} | placeholders))
    ...
    def process_messages(
        ...
        if len(messages) > 1 and not self.with_critic_in_the_loop:
            print(f"Got than one message to process. Num of messages: {len(messages)}.")
            raise ValueError("No messages to process.")
    ...
    def step(
        ...
        if assistant_response.terminated or assistant_response.msgs is None:
            return (
                ChatAgentResponse(
                    [assistant_response.msgs], assistant_response.terminated, assistant_response.info
                ),
